...
async def opportunities(establishment_id: str, auth: AuthContext) -> list[dict[str, Any]]:
    ...
    return rows_payload(await gateway.rest("oportunidades_crescimento", token=auth.token, params={"estabelecimento_id": f"eq.{establishment_id}", "status": "eq.aberta", "select": "*", "order": "prioridade.asc,detectada_em.desc", "limit": "100"}))
...
async def list_goals(establishment_id: str, auth: AuthContext) -> list[dict[str, Any]]:
    ...
    goals = rows_payload(await gateway.rest("metas_crescimento", token=auth.token, params={"estabelecimento_id": f"eq.{establishment_id}", "select": "*,profissionais(nome)", "order": "status.asc,periodo_fim.asc", "limit": "100"}))
    for goal in goals:
...
async def create_goal(payload: GoalCreate, auth: AuthContext) -> dict[str, Any]:
    ...
    rows = rows_payload(await gateway.rest("metas_crescimento", method="POST", token=auth.token, json=data, headers={"Prefer": "return=representation"}))
    if not rows:
...
async def update_goal(goal_id: str, payload: GoalUpdate, auth: AuthContext) -> dict[str, Any]:
    ...
    rows = rows_payload(await gateway.rest("metas_crescimento", method="PATCH", token=auth.token, params={"id": f"eq.{goal_id}"}, json=data, headers={"Prefer": "return=representation"}))
    if not rows:
