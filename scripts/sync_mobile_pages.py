...
def transform(source: Path) -> str:
    ...
    private_source = bool(re.search(r'<meta\b[^>]*content=["\']noindex,nofollow["\'][^>]*name=["\']robots["\'][^>]*>', html, re.I))
    robots_value = "noindex,nofollow" if private_source else "noindex,follow"
    mobile_meta = f'<meta content="yes" name="mobile-web-app-capable"/><meta content="{robots_value}" name="robots"/><link href="{canonical}" rel="canonical"/>'
    # Remove any source/generated robots directive and canonical before adding
    # exactly one mobile directive. Duplicate, conflicting meta tags make the
    # crawler behavior undefined and obscure whether a private page can follow.
    html = re.sub(r'<meta\b[^>]*\bname=["\']robots["\'][^>]*>', "", html, flags=re.I)
    html = re.sub(r'<link\b[^>]*href=["\']https://barberhuboficial\.vercel\.app/html/[^"\']+["\'][^>]*rel=["\']canonical["\'][^>]*>', "", html, flags=re.I)
    html = html.replace('<meta content="yes" name="mobile-web-app-capable"/>', "")
    # As folhas específicas do app vêm depois da base desktop. A 1.11 deve ser
    # sempre a última para que correções atuais não sejam sobrescritas por
