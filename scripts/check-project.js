/**
 * Auditoria local do Barber Hub 1.10.1.
 *
...
const required = [
  ...
  "js/backend-api.js",
  "js/features/professional-operation.js",
  "js/features/retention-growth.js",
...
const migration17 = fs.readFileSync(path.join(root, "sql/17_correcao_auditoria_seguranca.sql"), "utf8");
for (const expected of ["suspenso_pela_moderacao", "validar_transicao_status_agendamento", "pg_advisory_xact_lock", "curtidas_count", "commit;"]) {
  if (!migration17.toLowerCase().includes(expected.toLowerCase())) errors.push(`Migration 17 não contém: ${expected}`);
}
...
if (!serviceWorker.includes("barberhub-v1.10.1-mobile-r1")) errors.push("Service Worker não usa o cache atual da versão 1.10.1.");
...
console.log(`Barber Hub 1.10.1: ${required.length} arquivos centrais encontrados.`);
